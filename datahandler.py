...
AUSTRIAN_REGIONAL_CAPITALS_UNICODE = ["Wien", "St. P&ouml;lten", "Eisenstadt", "Graz",
...
def get_average_temperature_of_country():
    ...
    for city in AUSTRIAN_REGIONAL_CAPITALS_UNICODE:
        city_temperature_sum += scraper.get_temperature_from_city(city)
    view.clear_screen()
    return str(city_temperature_sum / len(AUSTRIAN_REGIONAL_CAPITALS_UNICODE))[0:5]


def get_temperature_of_city(city_index):
    view.print_waiting_message()
    chosen_city = AUSTRIAN_REGIONAL_CAPITALS_UNICODE[city_index]
...
def get_temperatures_for_all_cities():
    temperature_dict = {}
    for index, city in enumerate(AUSTRIAN_REGIONAL_CAPITALS_UNICODE):
...
def get_city_with_highest_temperature():
    ...
    hottest_cities_dict = {}
    highest_temp = 0
    for index, key in enumerate(temperature_dict):
        if temperature_dict[key] > highest_temp:
            highest_temp = temperature_dict[key]
            hottest_cities_dict = {}
            hottest_cities_dict[index] = temperature_dict[key]
